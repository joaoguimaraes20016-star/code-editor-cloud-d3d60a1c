...
-- Update the close_deal_transaction function to assign MRR schedules to the closer
...
BEGIN
  ...
  IF p_mrr_amount > 0 AND p_mrr_months > 0 THEN
    ...
    -- Create new MRR schedule with assigned_to set to the closer
...
-- Also update the existing MRR schedule to assign it to the closer
UPDATE mrr_schedules
SET assigned_to = (
  SELECT closer_id 
  FROM appointments 
  WHERE appointments.id = mrr_schedules.appointment_id
)
