CREATE OR REPLACE FUNCTION public.close_deal_transaction(p_appointment_id uuid, p_closer_id uuid, p_cc_amount numeric, p_mrr_amount numeric, p_mrr_months integer, p_product_name text, p_notes text DEFAULT NULL::text, p_closer_name text DEFAULT NULL::text, p_closer_commission_pct numeric DEFAULT 10.0, p_setter_commission_pct numeric DEFAULT 5.0)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_sale_date date;
  v_mrr_assignee_id uuid;
  v_team_mrr_assignment text;
BEGIN
  ...
  IF p_mrr_amount > 0 AND p_mrr_months > 0 THEN
    ...
    FOR i IN 0..(p_mrr_months - 1) LOOP
      ...
      -- Only create setter MRR commission if there's a setter
      IF v_appointment.setter_id IS NOT NULL THEN
        INSERT INTO mrr_commissions (
          team_id,
          sale_id,
    ...
    -- Determine MRR schedule assignee based on team config
    IF v_team_mrr_assignment = 'closer_who_closed' THEN
      v_mrr_assignee_id := p_closer_id;
    ELSIF v_team_mrr_assignment = 'original_setter' THEN
      v_mrr_assignee_id := v_appointment.setter_id;
    ELSIF v_team_mrr_assignment = 'appointment_closer' THEN
      v_mrr_assignee_id := v_appointment.closer_id;
    ELSE
      v_mrr_assignee_id := p_closer_id;
    END IF;
