-- Update create_task_with_assignment to include pipeline_stage for follow-up tasks
...
DECLARE
  ...
  appointment_pipeline_stage TEXT;
BEGIN
  -- Get appointment details
  SELECT setter_id, closer_id, start_at_utc, pipeline_stage
  INTO existing_setter, existing_closer, appointment_start, appointment_pipeline_stage
  FROM appointments
  ...
  -- Determine target role and due_at based on task type
  ...
  ELSIF p_task_type = 'follow_up' THEN
    ...
    -- For follow-up tasks, due_at is the follow_up_date at 10am
    calculated_due_at := (p_follow_up_date::timestamp + INTERVAL '10 hours')::timestamptz;
    
  ELSIF p_task_type = 'reschedule' THEN
    target_role := COALESCE(team_default_routing->>'reschedule', 'closer');
    -- For reschedule tasks, due_at is the reschedule_date at 10am (or now if no date)
    calculated_due_at := COALESCE(
      (p_reschedule_date::timestamp + INTERVAL '10 hours')::timestamptz,
      NOW()
    );
  ELSE
    target_role := 'closer';
    calculated_due_at := NOW();
  END IF;
  ...
  INSERT INTO confirmation_tasks (
    ...
    target_role,
    CASE 
      WHEN p_task_type = 'follow_up' THEN appointment_pipeline_stage
      ELSE NULL 
    END,
    CASE 
      WHEN p_task_type = 'follow_up' THEN 1
      ELSE NULL 
    END
  )
